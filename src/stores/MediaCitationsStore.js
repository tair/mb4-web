...
export const useMediaCitationsStore = defineStore({
  id: 'media-citations',
  state: () => ({
    isLoaded: false,
    map: new Map(),
  ...
  actions: {
    async fetchCitations(projectId, mediaId) {
      const response = await apiService.get(`/projects/${projectId}/media/${mediaId}/citations`)
    ...
    async create(projectId, mediaId, citationData) {
      try {
        const response = await apiService.post(`/projects/${projectId}/media/${mediaId}/citations/create`, { citation: citationData })
        const responseData = await response.json()
    ...
    async edit(projectId, mediaId, citationId, citation) {
      try {
        const response = await apiService.post(`/projects/${projectId}/media/${mediaId}/citations/${citationId}/edit`, { citation })
        if (response.ok) {
          const responseData = await response.json()
    ...
    async deleteIds(projectId, mediaId, citationIds) {
      const response = await apiService.post(`/projects/${projectId}/media/${mediaId}/citations/delete`, {
