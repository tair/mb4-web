...
onMounted(() => {
  const modal = document.getElementById('taxaDeleteModal')
  ...
  <div class="modal" id="taxaDeleteModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body" v-if="usages == null">
          Loading...
        </div>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="taxon in taxa" :key="taxon.taxon_id">
                <td><TaxonomicName :showExtinctMarker="true" :taxon="taxon" /></td>

                <td v-if="taxon.taxon_id in usages">
                  This taxon is affiliated with {{getUsageText(usages[taxon.taxon_id])}}.
                  <button
                    type="button"
                  ...
                  <div :id="`collapseTransferTaxon-${taxon.taxon_id}`" class="collapse">
                    <div class="card card-body">
                      <TaxaSearchInput :search="searchTaxa" @select="(t) => setRemappedTaxonId(taxon.taxon_id, t?.taxon_id)">
                        <template #item="taxon">
