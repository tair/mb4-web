...
onMounted(async () => {
  ...
  taxa.value = response.data.taxaLength
  characters.value = response.data.characterLength
  onetimeMedia.value = response.data.onetimeMedia
  media.value = response.data.mediaLength
...
<template>
  <LoadingIndicator :isLoaded="isLoaded">
    ...
    <p>
      Note that the new project you create here will be separate from P4091
      (Test) and create only the subset of information described above. Changes
      made to the newly created project will not affect P4091 or vice versa.
    </p>
    <p>
