  ...
  export default {
    ...
    methods: {
      ...
      async handleProject(projectId) {
        try {
          const response = await axios.get(`${import.meta.env.VITE_API_URL}/public/projects/${projectId}`,);
          const project = response.data;
  
          if (project.published === 1) {
            window.location.href = `${import.meta.env.VITE_HOST}/project/${folio.project_id}/overview`;
          } else {
      ...
      async handleFolio(folioId) {
        try {
          const folioResponse = await axios.get(`${import.meta.env.VITE_API_URL}/public/projects/folios/${folioId}`);
          const folio = folioResponse.data;
  
          if (folio) {
            window.location.href = `${import.meta.env.VITE_HOST}/project/${folio.project_id}/folios/${folio.folio_id}`;
          } else {
